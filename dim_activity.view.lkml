view: dim_activity {
  ...
  set: curated_fields_PM {fields:[APPLICATIONNAME,count_gradable,status,cengage_app,activity_app_type]}
...
#           ;;
  ...
  dimension: cengage_app {
    group_label: "Activity App Category"
  ...
  dimension: activity_app_type {
    group_label: "Activity Type Category"
    label: "activity app category"
    sql: CASE WHEN ${TABLE}.APPLICATIONNAME  IN ('ADAPTIVE','APLIA','APLIAMOBILE','CNOW.HW','SAM.APPIFICATION.PROD','WEBASSIGN','CENGAGE.ASSIGNMENT','CENGAGE.MINDTAPCASACTIVITYPLAYER','CEREGO',
            'MINDAPP-GROVE','MINDAPP-CXP','TURNITIN','CENGAGE.INSITE')
            OR ${TABLE}.APPLICATIONNAME ilike 'DLMT%'
          THEN 'Assessment'
           WHEN ${TABLE}.APPLICATIONNAME  IN ('CENGAGE.FLASHCARD','CENGAGE.READER','CENGAGE.READER.MT4','CENGAGE.RSSFEED','FLASHNOTES','CENGAGE.RSSFEED','LAMS','LAMS_V2','CENGAGE.QUESTIA','MINDAPP-SCENARIO',
          'WRITE-EXPERIENCE','EMPLOYER.CONNECTION','MILADY.PROCEDURAL.TRACKER','EMPLOYERCONNECTION')
          THEN 'Narrative'
           WHEN ${TABLE}.APPLICATIONNAME  IN ('WEBVIDEO.APP','MOBLABGAMES','CENGAGE.MEDIA','COGLAB','CENGAGE.STUDYCENTER','CENGAGE.DIET.WELLNESS.PLUS','CENGAGE.YOUSEEU','CENGAGE.NATIVE.DOWNLOADABLEFILE',
            'CENGAGE.GOOGLE.DOC','CENGAGE.OUTLINE.SPEECH','CENGAGE.WEBLINK','CENGAGE.ALG','CENGAGE.DISCUSSIONBOARD','CENGAGE.SPEECHVIDEOLIBRARYPROD','CENGAGE.ONEDRIVE','MIXOFFICE','CENGAGE.NON-MT-ACTIVITY',
            'CENGAGE.KALTURA','MILADY.THINK.BEYOND','MINDAPP-AB','MILADY.THINK.BEYOND')
          THEN 'Media'
          ELSE 'Unknown'
          END;;
  }
  ...
  dimension: graded_app_type {
    label: "Gradable App Category"
